...
void _ijon_decrypt_round(uint32_t* _x, uint32_t* _y, uint32_t* _z, uint32_t* _w, uint32_t const* subkeys) {
    uint32_t x = *_x, y = *_y, z = *_z, w = *_w;

    _ijon_pbox_inv(&x, &y, &z, &w);
...
void ijon_decrypt_block(uint8_t const* ciphertext, uint8_t const* key_exp, uint8_t* plaintext) {
    uint32_t const* c = (uint32_t const*) ciphertext; // 4 x 32 bit
    uint32_t const* k_exp = (uint32_t const*) key_exp; // 80 x 32 bit
    uint32_t* p = (uint32_t*) plaintext; // 4 x 32 bit

    // Copy ciphertext to state
    uint32_t state[IJON_BLOCK_BYTES / 4];
    memcpy(state, c, IJON_BLOCK_BYTES);

    // Perform 10 steps
    for(int step = 0; step < IJON_STEPS; step++) {
        _ijon_decrypt_round(state + 0, state + 1, state + 2, state + 3, k_exp + (8 * (IJON_STEPS - step - 1)));
    }

    // Copy state to plaintext
    memcpy(p, state, IJON_BLOCK_BYTES);
}
